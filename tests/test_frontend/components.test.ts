...
describe.each(cases)("GraphView ($name)", ({ graph }) => {
  it("renders all node labels without corpus-specific assumptions", () => {
    ...
    render(<GraphView graph={graph} selectedNodeId={null} onSelectNode={onselect} />);
...
describe.each(cases)("Sidebar ($name)", ({ graph }) => {
  it("renders the node label, type, summary, and source_refs", () => {
    const node = graph.nodes[0];
    render(<Sidebar corpusId={graph.corpus_id} node={node} />);
    expect(screen.getByText(node.label)).toBeTruthy();
  ...
  it("does not assume a spouse field exists", () => {
    ...
    const node = graph.nodes[0];
    const { container } = render(<Sidebar corpusId={graph.corpus_id} node={node} />);
